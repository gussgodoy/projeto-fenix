...
def handle_our_keys():
    ...
    try:
        with conn.cursor() as cur:
            if request.method == 'GET':
                sql = """
                    SELECT k.id, k.service_name, k.api_key, k.status_id, s.color as status_color 
                    FROM our_api_keys k 
...
# --- ROTAS DE IA (PROVEDORES E CHAVES) ---
    ...
    try:
        with conn.cursor() as cur:
            if request.method == 'GET':
                ...
                cur.execute("""
                    SELECT k.id, k.provider_id, k.key_name, k.api_key, k.status_id, s.name as status_name, s.color as status_color 
                    FROM ia_api_keys k 
                    JOIN statuses s ON k.status_id = s.id 
                    ORDER BY k.key_name
                """)
    ...
    try:
        with conn.cursor() as cur:
            ...
            for perfil in perfis:
                if perfil.get('config_avancada') and isinstance(perfil.get('config_avancada'), str):
